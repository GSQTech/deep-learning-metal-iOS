...
namespace caffe {
    ...
    protected:
        ...
        blob_bottom_2_(new Blob<Dtype>(1, 3, 6, 4)),
    ...
    TYPED_TEST(ConvolutionLayerTest, TestSimpleConvolution) {
        ...
        Dtype* bias = layer->blobs()[1]->mutable_cpu_data();
        ...
        for (int i = 0; i < this->blob_top_->count(); ++i) {
            std::cout << "The i now is "<< i << " Left is " << top_data[i] << " Right is " <<ref_top_data[i] << std::endl;
