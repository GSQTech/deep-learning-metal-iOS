...
//  set up metal
    NSString *funcname = [NSString stringWithCString:this->layer_param().name().c_str() encoding:[NSString defaultCStringEncoding]];
    ...
    for (int i = 0; i < bottom.size(); ++i) {
        ...
        id<MTLBuffer> in_gpu    = [device newBufferWithBytesNoCopy:bottom[i]->mutable_cpu_data()  length:align_size(bottom[i]->count()*4)  options:MTLStorageModeShared deallocator:nil];
