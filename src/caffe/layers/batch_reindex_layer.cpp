...
#ifdef METAL
template <typename Dtype>
void BatchReindexLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
   
    metal_wrapper3("BatchReindexForward", bottom[0]->mutable_cpu_data(), bottom[0]->count() * 4, top[0]->mutable_cpu_data(), top[0]->count(), bottom[1]->mutable_cpu_data(), bottom[1]->count() * 4, {top[0]->count(), bottom[0]->count() / bottom[0]->shape(0)}, {}, top[0]->count());
}

template <typename Dtype>
void BatchReindexLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top, const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom){
