...
void SliceLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
    ...
    for (int i = 0; i < top.size(); ++i) {
        ...
        metal_wrapper("Slice", bottom[0]->mutable_gpu_data(), bottom[0]->count()*4, top[0]->mutable_gpu_data(), top[0]->count()*4, {top_slice_size * num_slices_, kForward, num_slices_, slice_size_, bottom[0]->shape(slice_axis_), top[i]->shape(slice_axis_), offset_slice_axis}, {}, top_slice_size * num_slices_);
        offset_slice_axis += top_slice_axis;
    }
}

template <typename Dtype>
void SliceLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top, const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom){
