...
void LRNLayer<Dtype>::CrossChannelForward_gpu(
   ...
   // const Dtype* bottom_data = bottom[0]->gpu_data();
    ...
    metal_wrapper("LRNFillScale", bottom[0]->mutable_cpu_data(), bottom[0]->count()*4, scale_data, scale_.count()*4, {n_threads, channels_, height_, width_, size_}, {alpha_/size_, k_}, n_threads);
    n_threads = bottom[0]->count();
    

    metal_wrapper3("LRNComputeOutput", bottom[0]->mutable_cpu_data(), bottom[0]->count() * 4, top[0]->mutable_cpu_data(), top[0]->count(), scale_data, scale_.count() * 4, {top[0]->count()}, {-beta_}, top[0]->count());
}
