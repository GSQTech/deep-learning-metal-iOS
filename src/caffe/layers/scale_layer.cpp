...
void ScaleLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
    ...
    //metal_wrapper("ReLUForward", bottom[0]->mutable_gpu_data(), bottom[0]->count()*4, top[0]->mutable_gpu_data(), top[0]->count()*4, {bottom[0]->count()}, {this->layer_param_.relu_param().negative_slope()}, bottom[0]->count());
    ...
    metal_wrapper3("ScaleForward", bottom[0]->mutable_cpu_data(), bottom[0]->count()*4, top[0]->mutable_cpu_data(), top[0]->count()*4, scale_data, scale_size, {top[0]->count(), scale_dim_, inner_dim_}, {}, top[0]->count());
    
    if (bias_layer_) {
        bias_layer_->Forward(bias_bottom_vec_, top);
        //metal_wrapper4("ScaleBiasForward", bottom[0]->mutable_cpu_data(), bottom[0]->count()*4, top[0]->mutable_cpu_data(), top[0]->count()*4, scale_data, scale_size, this->blobs_[bias_param_id_]->mutable_cpu_data(), this->blobs_[bias_param_id_]->count() * 4, {top[0]->count(), scale_dim_, inner_dim_}, {}, top[0]->count());
    } else {
        //metal_wrapper3("ScaleForward", bottom[0]->mutable_cpu_data(), bottom[0]->count()*4, top[0]->mutable_cpu_data(), top[0]->count()*4, scale_data, scale_size, {top[0]->count(), scale_dim_, inner_dim_}, {}, top[0]->count());
...
template <typename Dtype>
void ScaleLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top, const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom){
