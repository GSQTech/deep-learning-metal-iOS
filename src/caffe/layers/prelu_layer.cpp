...
void PReLULayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
    ...
    id<MTLFunction>     function = [library newFunctionWithName:@"PReLUForward"];
    ...
    id<MTLBuffer> slope_gpu = [device newBufferWithBytesNoCopy:this->blobs_[0]->mutable_cpu_data()  length:align_size(this->blobs_[0]->count()*4)  options:MTLStorageModeShared deallocator:nil];
    ...
    int__ = [device newBufferWithLength:4*4 options:MTLStorageModeShared];
    int* p_tmp_int = (int *)[int__ contents];
    p_tmp_int[0] = count;
    p_tmp_int[1] = bottom[0]->channels();
    p_tmp_int[2] =  bottom[0]->count(2);
    p_tmp_int[3] = channel_shared_ ? bottom[0]->channels() : 1;;
    ...
    [encoder setBuffer:slope_gpu       offset:0 atIndex:3];
    ...
    metal_wrapper3("PReLUForward", bottom[0]->mutable_cpu_data(), bottom[0]->count() * 4, top[0]->mutable_cpu_data(), top[0]->count(), this->blobs_[0]->mutable_cpu_data(), this->blobs_[0]->count() * 4, {top[0]->count(), bottom[0]->channels(), bottom[0]->count(2), (channel_shared_ ? bottom[0]->channels() : 1)}, {}, top[0]->count());
}

template <typename Dtype>
void PReLULayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top, const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom){
