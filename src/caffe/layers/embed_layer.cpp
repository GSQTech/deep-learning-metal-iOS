...
void EmbedLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
                                    const vector<Blob<Dtype>*>& top) {
    metal_wrapper3("crop_kernel_forward", bottom[0]->mutable_cpu_data(), bottom[0]->count() * 4, top[0]->mutable_cpu_data(), top[0]->count(), this->blobs_[0]->mutable_cpu_data(), this->blobs_[0]->count() * 4, {top[0]->count(), N_}, {}, top[0]->count());

    if (bias_term_) {
        caffe_gpu_gemm_metal<Dtype>(CblasNoTrans, CblasNoTrans, M_, N_, 1, Dtype(1),
                              bias_multiplier_.gpu_data(), 0,
                              this->blobs_[1]->gpu_data(), 0, Dtype(1), top[0]->mutable_cpu_data(), 0);
    }
}

template <typename Dtype>
void EmbedLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top, const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom){
