...
void CropLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
    ...
    id<MTLFunction>     function = [library newFunctionWithName:@"CropForward"];
    
    
    id<MTLBuffer> data_input  = [device newBufferWithBytesNoCopy:bottom[0]->mutable_cpu_data()  length:align_size(bottom[0]->count()*4)  options:MTLStorageModeShared deallocator:nil];
    id<MTLBuffer> data_output = [device newBufferWithBytesNoCopy:top[0]->mutable_cpu_data() length:align_size(top[0]->count()*4)  options:MTLStorageModeShared deallocator:nil];
    
    id<MTLBuffer> src_strides_gpu = [device newBufferWithBytesNoCopy:src_strides_.mutable_cpu_data() length:align_size(src_strides_.count()*4)  options:MTLStorageModeShared deallocator:nil];
    
    id<MTLBuffer> dest_strides_gpu = [device newBufferWithBytesNoCopy:dest_strides_.mutable_cpu_data() length:align_size(dest_strides_.count()*4)  options:MTLStorageModeShared deallocator:nil];
    
    id<MTLBuffer> offsets_gpu = [device newBufferWithBytesNoCopy:offsets.mutable_cpu_data() length:align_size(offsets.count()*4)  options:MTLStorageModeShared deallocator:nil];
    ...
    int__ = [device newBufferWithLength:2*4 options:MTLStorageModeShared];
    int* p_tmp_int = (int *)[int__ contents];
    p_tmp_int[0] = count;
    p_tmp_int[1] = bottom[0]->num_axes();

        
    
    [encoder setBuffer:int__       offset:0 atIndex:0];
    [encoder setBuffer:src_strides_gpu       offset:0 atIndex:1];
    [encoder setBuffer:dest_strides_gpu       offset:0 atIndex:2];
    [encoder setBuffer:offsets_gpu       offset:0 atIndex:3];
    [encoder setBuffer:data_input       offset:0 atIndex:4];
    [encoder setBuffer:data_output       offset:0 atIndex:5];
...
template <typename Dtype>
void CropLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top, const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom){
