...
#ifdef METAL
template <typename Dtype>
void TileLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
    const Dtype* bottom_data = bottom[0]->gpu_data();
    Dtype* top_data = top[0]->mutable_gpu_data();
    const int bottom_tile_axis = bottom[0]->shape(axis_);
    const int nthreads = top[0]->count();
    
    metal_wrapper("Tile", bottom[0]->mutable_gpu_data(),bottom[0]->count()*4, top[0]->mutable_gpu_data(), top[0]->count()*4, {top[0]->count(), static_cast<int>(inner_dim_), static_cast<int>(tiles_), bottom_tile_axis}, {}, top[0]->count());

}

template <typename Dtype>
void TileLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top, const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom){
