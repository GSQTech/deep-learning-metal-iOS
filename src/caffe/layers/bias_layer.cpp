...
//    id<MTLFunction>     function = [library newFunctionWithName:@"BiasForward"];
//
//
...
//    id<MTLBuffer> data_output = [device newBufferWithBytesNoCopy:top[0]->mutable_cpu_data() length:align_size(top[0]->count()*4)  options:MTLStorageModeShared deallocator:nil];
    ...
    int bias_size =
    ((bottom.size() > 1) ? bottom[1] : this->blobs_[0].get())->count() * 4;
    
//    id<MTLBuffer> bias_gpu = [device newBufferWithBytesNoCopy:bias_data  length:align_size(bias_size)  options:MTLStorageModeShared deallocator:nil];
//
//
...
//
//
//    int__ = [device newBufferWithLength:3*4 options:MTLStorageModeShared];
//    int* p_tmp_int = (int *)[int__ contents];
//    p_tmp_int[0] = count;
//    p_tmp_int[1] = bias_dim_;
//    p_tmp_int[2] = inner_dim_;
//
//
//
//
...
//    [encoder setBuffer:data_output       offset:0 atIndex:3];
//
//
...
//    [buffer waitUntilCompleted];
    ...
    metal_wrapper3("BiasForward", bottom[0]->mutable_cpu_data(), bottom[0]->count()*4, top[0]->mutable_cpu_data(), top[0]->count()*4, bias_data, bias_size, {top[0]->count(), bias_dim_, inner_dim_}, {}, top[0]->count());
}

template <typename Dtype>
void BiasLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top, const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom){
