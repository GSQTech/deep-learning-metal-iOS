...
void MVNLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
    ...
    // subtract mean
    caffe_gpu_gemv_metal<Dtype>(CblasNoTrans, num, dim, 1. / dim, bottom_data,
                          sum_multiplier_.gpu_data(), 0., mean_.mutable_gpu_data());  // EX
    caffe_gpu_gemm_metal<Dtype>(CblasNoTrans, CblasNoTrans, num, dim, 1, -1.,
                          mean_.gpu_data(), 0, sum_multiplier_.gpu_data(), 0, 0.,
                          temp_.mutable_gpu_data(), 0);

    metal_wrapper3("add_kernel", bottom[0]->mutable_cpu_data(), bottom[0]->count() * 4, top[0]->mutable_cpu_data(), top[0]->count() * 4, temp_.mutable_cpu_data(), temp_.count() * 4, {top[0]->count()}, {}, top[0]->count());

    
    if (this->layer_param_.mvn_param().normalize_variance()) {
        // compute variance using var(X) = E((X-EX)^2)
        metal_wrapper("powx_kernel", top[0]->mutable_cpu_data(), top[0]->count() * 4, temp_.mutable_cpu_data(), temp_.count() * 4, {bottom[0]->count()}, {Dtype(2)}, bottom[0]->count());
        
        caffe_gpu_gemv_metal<Dtype>(CblasNoTrans, num, dim, 1. / dim, temp_.gpu_data(),
                              sum_multiplier_.gpu_data(), 0.,
                              variance_.mutable_gpu_data());  // E((X-EX)^2)
        
        // normalize variance
        metal_wrapper("powx_kernel", variance_.mutable_cpu_data(), variance_.count() * 4, variance_.mutable_cpu_data(), variance_.count() * 4, {variance_.count()}, {Dtype(0.5)}, variance_.count());
        ...
        caffe_gpu_gemm_metal<Dtype>(CblasNoTrans, CblasNoTrans, num, dim, 1, 1.,
                              ...
                              temp_.mutable_gpu_data(), 0);
        metal_wrapper3("div_kernel", top[0]->mutable_cpu_data(), top[0]->count() * 4, temp_.mutable_cpu_data(), temp_.count() * 4, top[0]->mutable_cpu_data(), top[0]->count() * 4, {top[0]->count()}, {}, top[0]->count());
    }
}
