...
void ExpLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
    ...
    metal_wrapper("ExpForward", bottom[0]->mutable_gpu_data(), bottom[0]->count()*4, top[0]->mutable_gpu_data(), top[0]->count()*4, {bottom[0]->count()}, {this->inner_scale_, this->outer_scale_}, bottom[0]->count());
    
    //Forward_cpu(bottom, top);
}

template <typename Dtype>
void ExpLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top, const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom){
