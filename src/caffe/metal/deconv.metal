...
kernel void
deconv_no_bias(device Dtype *im_in  [[buffer(0)]], const device Dtype *im_out [[buffer(1)]], const device Dtype *wg     [[buffer(2)]], const constant int* int__[[buffer(4)]], uint3 tid[[thread_position_in_threadgroup]],uint3 off[[threadgroup_position_in_grid]]) {
    ...
    int v_B_off = int__[3];
    int v_C_off = int__[4];
    int v_imsi_0 = int__[5];
    int v_imso_0 = int__[6];
    int v_imsi_1 = int__[7];
    int v_imso_1 = int__[8];
    int v_imsi = int__[9];
    int v_imso = int__[10];
    int v_k_0 = int__[11];
    int v_k_1 = int__[12];
    ...
    const device Dtype* Bptr = im_out + v_B_off * batch;
    device Dtype* Cptr = im_in + v_C_off * batch;
    {
        Dtype4 Creg[4][1];
#pragma unroll
        for (int wm=0; wm<4; ++wm) {
#pragma unroll
            for (int wn=0; wn<1; ++wn) {
...
kernel void
deconv_bias(device Dtype *im_in  [[buffer(0)]], const device Dtype *im_out [[buffer(1)]], const device Dtype *wg     [[buffer(2)]], const device Dtype *bias     [[buffer(3)]], const constant int* int__[[buffer(4)]], uint3 tid[[thread_position_in_threadgroup]],uint3 off[[threadgroup_position_in_grid]]) {
